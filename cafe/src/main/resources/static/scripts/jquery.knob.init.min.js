!function (e) {
    "use strict";
    e(document).ready(function () {
        e(".knob").knob({
            draw: function () {
                if ("tron" == this.$.data("skin")) {
                    this.cursorExt = .3;
                    var e, t = this.arc(this.cv), s = 1;
                    return this.g.lineWidth = this.lineWidth, this.o.displayPrevious && (e = this.arc(this.v), this.g.beginPath(), this.g.strokeStyle = this.pColor, this.g.arc(this.xy, this.xy, this.radius - this.lineWidth, e.s, e.e, e.d), this.g.stroke()), this.g.beginPath(), this.g.strokeStyle = s ? this.o.fgColor : this.fgColor, this.g.arc(this.xy, this.xy, this.radius - this.lineWidth, t.s, t.e, t.d), this.g.stroke(), this.g.lineWidth = 2, this.g.beginPath(), this.g.strokeStyle = this.o.fgColor, this.g.arc(this.xy, this.xy, this.radius - this.lineWidth + 1 + 2 * this.lineWidth / 3, 0, 2 * Math.PI, !1), this.g.stroke(), !1
                }
            }
        });
        var t, s = 0, i = 0, n = 0, o = e("div.idir"), a = e("div.ival"), r = function () {
            n++, o.show().html("+").fadeOut(), a.html(n)
        }, l = function () {
            n--, o.show().html("-").fadeOut(), a.html(n)
        };
        return e("input.infinite").knob({
            min: 0, max: 20, stopper: !1, change: function () {
                t > this.cv ? s ? (l(), s = 0) : (s = 1, i = 0) : t < this.cv && (i ? (r(), i = 0) : (i = 1, s = 0)), t = this.cv
            }
        }), !1
    })
}(jQuery);